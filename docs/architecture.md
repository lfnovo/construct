...
Location-level Git freshness remains behind the same native read-only boundary.
...
- deterministic document enumeration by structured metadata, relative path,
  and generation-bound opaque cursors without a full-text query;
...
Location allowlist and performs one best-effort initial reconciliation. The MCP
adapter owns no timer or background reconciliation loop. Before an indexed
